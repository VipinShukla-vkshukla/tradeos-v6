...
```
   1  Both frameworks (default)     4  Check readiness    7  IP vs allowlist
   2  Swing only                    5  Status             E  Evening pipeline
   3  Intraday only                 6  Health sweep       K  KILL SWITCH

   LEARN (measures, proposes, changes nothing)
     L  Weekly review     D  Discover engines     P  Open proposals

   ORACLE SERVER
     8  Logs     9  Update (pull+restart)     N  Status     X  Stop
```
