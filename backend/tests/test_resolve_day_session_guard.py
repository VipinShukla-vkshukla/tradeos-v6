...
def _migrated(rows: list[dict]) -> list[dict]:
    """The same fixture rows, on a book where migration 080 HAS been applied.
...
def test_provenance_is_dropped_whole_when_the_migration_is_unapplied():
    ...
    assert res["resolved"] == 5, (
        f"resolved {res['resolved']} of 5 with migration 080 unapplied — the "
...
def test_provenance_names_exactly_what_migration_080_adds():
    ...
    sql = (Path(__file__).resolve().parent.parent / "db" / "migrations"
           / "080_outcome_scoring_provenance.sql").read_text(encoding="utf-8")
    for col in PROVENANCE_COLS:
        assert f"ADD COLUMN IF NOT EXISTS {col}" in sql, (
            f"code writes {col} and migration 080 does not add it")
...
TESTS = [
    ...
    ("provenance names exactly what migration 080 adds",
     test_provenance_names_exactly_what_migration_080_adds),
]
