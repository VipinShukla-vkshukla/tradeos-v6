...
def _bars(sb, symbols: list[str], since: str) -> dict[str, list[dict]]:
    ...
    high/low/close read here, and stock_data_daily's own retention is being
    cut well below this function's lookback window (migration 130). Verified
    live before this shipped: today's active universe has IDENTICAL
    per-symbol row counts in both tables over 120 days (0 short, 0 missing)
