...
**Version 4.3 · March 2026 · Supersedes all previous versions**
...
| `position_event_monitor.py` | Morning 8:35 AM: scans open positions for upcoming events, pre-market Telegram brief. **Now built.** | ✅ Built |
...
- ✅ `generate_shortlist.py` producing `ai_shortlist_rank` on `master_shortlist` (v4.3 fix: now correctly writes back to `master_shortlist`)
- ✅ `ingest_asm_gsm.py` populating `safety_lists` (v4.3 fix: v6 kill switch, dry-run support)
- ✅ `position_event_monitor.py` built and wired to `pipeline_morning.yml` at 8:35 AM IST
...
**Implements:** HIGH/MEDIUM/LOW tier classification, configurable day windows (`HIGH_DAYS=3`, `MEDIUM_DAYS=2`, `LOW_DAYS=5`), dual-source deduplication across `nifty_upcoming_events` + `event_calendar` by `(symbol, event_date)`, `data_anomalies` writes for HIGH (ERROR) and MEDIUM (WARN), Telegram brief for all tiers, `--days N` flag, `--dry-run`, kill switch, non-fatal throughout.

**Wire into `pipeline_morning.yml`** (after kite_reconcile):
```yaml
...
### 8:35 AM IST — Position Event Monitor (automated, Phase 1+, via `pipeline_morning.yml`)
1. `position_event_monitor.py` → scans open positions for upcoming events → pre-market Telegram brief (HIGH 🔴 / MEDIUM 🟡 / LOW 🟢)
...
└── backend/
    ...
    │   └── position_event_monitor.py ← P1 ✅ BUILT (v4.3 new — morning 8:35 AM event scanner)
    │
...
*TradeOS v6 · Golden Copy v4.3 · March 2026*
*Google Sheet ID: 1yclJSWpRtnenZcd3M1lKbYOnh9CGEnRbMCwlTwv-1Dw*
