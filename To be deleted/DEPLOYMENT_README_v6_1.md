...
**Critical ordering rule:** Steps 10.5 → 10.6 → 12 must be in this exact sequence.
`screen_stocks` populates the symbol list → `compute_msl` enriches them → `generate_signals` reads enriched `master_shortlist`.
...
## Phase 2.5 — Dynamic Screener + MSL Intelligence Engine
...
**`run_pipeline.py`**
- Add `step_screen_stocks` at position 10.5 and `step_compute_msl` at position 10.6
- Confirm ordering: screen_stocks MUST run before compute_msl
```python
("10.5_screen_stocks", step_screen_stocks, False),
