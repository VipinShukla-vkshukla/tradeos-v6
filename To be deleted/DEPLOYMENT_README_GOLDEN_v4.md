...
**Version 4.0 · March 2026 · Supersedes all previous versions**
...
**Pipeline runs:** 7:00 AM (global cues), 8:45 AM (Kite reconcile, Phase 2+), 6:00 PM (main), Sundays 6 AM (ML training).
...
══════════════════════════════════════════════════════════════
  NSE Direct Data                         Zerodha Kite
  ┌──────────────────┐                 ┌──────────────────┐
  │ Chartink CSV     │                 │ Live Prices      │
  │ Bhavcopy         │                 │ Order Placement  │
  │ FII/DII flows    │                 │ Holdings Sync    │
  │ NSE Events       │                 └────────┬─────────┘
  │ ASM/GSM lists    │                          │
  └─────────┬────────┘                          │
            │                                   │
  ...
  ┌─────────────────────────────────────────────────────┐
  │               Supabase (Database)                    │
  │  stock_data_daily · signal_log · open_positions      │
  │  lessons · market_regime · fii_dii_flow · ...        │
  └────────────┬────────────────────────────────────────┘
               │
       ┌───────┴───────┐
       ▼               ▼
  Rule Engine      AI/ML Layer
  CTL+SBS+TPO+EAP  Claude/DeepSeek
  generate_signals ai_enrich
  independent_scan post_trade_analysis
       │               │
       └───────┬───────┘
               ▼
        ┌─────────────┐     ┌──────────────────┐
        │  Telegram   │     │  Frontend HTML   │
        │  Alerts     │     │  Dashboard       │
        ...
        risk_manager → execution_engine
               ...
               ▼
        Self-Evolution
        evolution_tracker → discovery_engine
```
...
| `ai_router.py` | Routes to configured provider, budget tracking | ✅ Live |
| `providers/ml_provider.py` | Local RandomForest conviction (free fallback) | ✅ Live |
| `providers/claude/openai/gemini/deepseek/grok_provider.py` | Commercial AI providers | ✅ Live |
...
| `ingest_global_cues.py` | 8 AM: Gift Nifty, USD/INR, crude → Telegram brief | ✅ Live |
| `independent_scanner.py` | 5 pattern scans parallel to rule engine | ✅ Live |
| `send_alerts.py` | Telegram evening digest with signals | ✅ Live |
| `pipeline_morning.yml` | GitHub Actions: 7 AM IST global cues | ✅ Running |
...
|-----|----------|------|---------------|
| G1  | ✅ Resolved | `generate_signals.py` | 9 ML feature columns missing from signal_log write |
| G2  | ✅ Resolved | `post_trade_analysis.py` | `outcome_pnl_pct` never written back to signal_log |
| G3  | 🟡 Moderate | `ai_enrich.py` | AI given empty computed fields with no warning |
| G4  | ✅ Resolved | `ai_enrich.py` + SQL | `ai_strategy_validation` not persisted to signal_log |
| G5  | ✅ Resolved | 4 provider files | JSON fence `\`\`\`json` not stripped before parsing |
| G6  | ✅ Resolved | `ai_enrich.py` | event_calendar never in AI context |
| G7  | ✅ Resolved | SQL only | `lessons` missing confidence, is_active, effectiveness cols |
| G8  | ✅ Resolved | `evolution_tracker.py` | v5 kill switch pattern (already in Fix #5 patch) |
| G9  | ✅ Resolved | `evolution_tracker.py` | Lessons fetched without dedup/filtering (in Fix #5 patch) |
| G10 | ✅ Resolved | `ingest_fii_dii.py` | Verify column names match DB (fixed by Fix #1 above) |
| G11 | ✅ Resolved | `pipeline_morning.yml` | Cron confirmed correct |
| G12 | ✅ Resolved | `pipeline_morning.yml` | Morning alert confirmed wired |
| G13 | ✅ Resolved | `ai_enrich.py` | sector_strength + industry_strength not in AI context |
| G14 | ✅ Resolved | `append_history.py` | `regime_history` snapshot never wired |
| G15 | ✅ Resolved | `evolution_tracker.py` | `ai_model_performance` not fed to evolution AI |
| G16 | ✅ Resolved | `ingest_global_cues.py` | S&P500 and change% columns missing |
...
#### PATCH G1 — Add 9 ML Feature Columns to signal_log (03.14.2026 Done)
...
#### PATCH G2 — Write outcome_pnl_pct Back to signal_log (03.14.2026 Done)
...
#### PATCH G3 — Zero-Data Prompt Guard (03.14.2026 Doesnt seems critical)
...
#### PATCH G5 — JSON Fence Strip (4 Files) (03.14.2026 Done)
...
#### PATCH G6 — event_calendar in AI Context (03.14.2026 Done)

**File:** `backend/ai/ai_enrich.py` — in context-building function: (03.14.2026  Change NOT Needed — Already Superseded with more advanced code)
```python
# G6 FIX
...
#### PATCH G7 — lessons Quality Columns (SQL Only) (03.14.2026 Done)
...
#### PATCH G13 — sector_strength + industry_strength in AI Context (03.14.2026  Change NOT Needed — Already Superseded with more advanced code)
...
#### PATCH G17 — market_regime + global_cues in AI Context (03.14.2026  Change NOT Needed — Already Superseded with more advanced code)
...
#### PATCH G18 — open_positions Portfolio in AI Context (03.14.2026  Change NOT Needed — Already Superseded with more advanced code)
...
**SQL:**
```sql
ALTER TABLE signal_log ADD COLUMN IF NOT EXISTS ai_strategy_validation TEXT; (03.14.2026 Done)
NOTIFY pgrst, 'reload schema';
```
...
#### PATCH G14 — Wire regime_history Snapshot (03.14.2026  Change NOT Needed — Already Superseded with more advanced code)
...
#### PATCH G15 — Feed ai_model_performance to evolution_tracker (03.14.2026  Change NOT Needed — Already Superseded with more advanced code)
...
#### PATCH G16 — S&P500 + Change % in global_cues (03.14.2026  Change NOT Needed — Already Superseded with more advanced code)
...
#### PATCH G10 — Verify FII/DII Column Names (03.14.2026  Change NOT Needed — Already Superseded with more advanced code)
...
[Y] G1:  SQL (9 cols) + generate_signals.py sig dict
[Y] G2:  post_trade_analysis.py outcome_pnl_pct  (in Fix #2 patch file)
[Y] G6:  ai_enrich.py — event_calendar in context
[Y] G17: ai_enrich.py — market_regime + global_cues in context
[Y] G18: ai_enrich.py — open_positions portfolio in context
[Y] G7:  SQL — lessons quality columns             (in SQL migration)
[Y] G10: Already fixed by Fix #1 ✅

Moderate (apply before Phase 2):
[N] G3:  ai_enrich.py — zero-data prompt guard
[Y] G5:  4 provider files — JSON fence strip
[Y] G13: ai_enrich.py — sector + industry in context
[Y] G8:  evolution_tracker.py kill switch          (in Fix #5 patch file)
[Y] G9:  evolution_tracker.py rich lessons context (in Fix #5 patch file)

Minor (apply before Phase 4):
[Y] G4:  ai_enrich.py + SQL — ai_strategy_validation
[Y] G14: append_history.py — regime_history snapshot
[Y] G15: evolution_tracker.py — provider performance in AI
[Y] G16: ingest_global_cues.py — S&P500 + change%
...
```sql
-- safety_lists table (ASM/GSM/FO_BAN)
...
**Path:** `backend/ingestion/compute_indicators.py`
...
### Step 2.3 — Build ingest_asm_gsm.py
...
### Step 2.4 — Build ml_regime_classifier.py

**Path:** `backend/ai/providers/ml_regime_classifier.py`
...
### Step 2.5 — Build data_quality_monitor.py

**Path:** `backend/ingestion/data_quality_monitor.py`
...
```
[ ] SQL: Phase 2 tables created (safety_lists, data_anomalies)
[ ] SQL: RENAME_MAP columns verified in stock_data_daily
[ ] compute_indicators.py built + tested (vol_ratio, ret_6m not NULL)
[ ] ingest_asm_gsm.py built + tested (safety_lists populated)
[ ] ml_regime_classifier.py built + trained (predicted_regime in market_regime)
[ ] data_quality_monitor.py built + running (data_anomalies getting rows)
[ ] run_pipeline.py updated with Phase 2 step order
...
### Step 4.2 — Build discovery_engine.py
...
[ ] discovery_engine.py built + tested
...
1. `ingest_global_cues.py` → fetches Gift Nifty, USD/INR, Brent, US markets → `global_cues`
2. `send_alerts.py --morning` → Telegram brief with gap signal, SL proximity alerts, open position risk summary
...
### 8:45 AM IST — Reconciliation (automated, Phase 2+, via updated `pipeline_morning.yml`)
1. `kite_reconcile.py` → compares Kite holdings vs Supabase open_positions → alerts on mismatches
...
Phase 0+:
  ...
  [Phase 2+]
  04 regime_predict       (non-fatal)   → market_regime.predicted_regime
  [All phases]
...
Phase 1+:
  ...
  [Phase 2+]
  09a   asm_gsm           (non-fatal)   → safety_lists
  [All Phase 1+]
  ...
  11/12 send_alerts       (non-fatal)   → Telegram evening digest
  [Phase 2+]
...
python alerts/send_alerts.py --morning
python alerts/send_alerts.py --test
python alerts/send_alerts.py --position-risk  # Phase 2+ pre-market position risk scan
```
...
### `pipeline_morning.yml` — Morning Pipeline (Updated for Phase 2)
**Job 1 cron:** `30 1 * * 1-5` (7:00 AM IST Mon–Fri) — Global cues + morning brief
**Job 2 cron:** `15 3 * * 1-5` (8:45 AM IST Mon–Fri, Phase 2+) — Kite reconciliation

**Job 1 steps:**
1. `python backend/ingestion/ingest_global_cues.py`
2. `python backend/alerts/send_alerts.py --morning`

**Job 2 steps (Phase 2+):**
1. `python backend/kite/kite_reconcile.py`
2. `python backend/alerts/send_alerts.py --position-risk`

Both jobs use `continue-on-error: true` — stale Kite token doesn't fail morning pipeline.
...
│   ├── pipeline_morning.yml         ← 7 AM + 8:45 AM IST weekdays (UPDATED for Kite reconcile)
...
│
└── backend/
    ├── run_pipeline.py              ← Main orchestrator (update for Phase 2 step order)
    ...
    │   ├── ingest_fii_dii.py        ← P1 ✅ ⚠️ PATCH Fix#1 REQUIRED
    │   ├── ingest_nse_events.py     ← P1 ✅
    │   ├── ingest_global_cues.py    ← P1 ✅ ⚠️ G16 patch needed
    │   ├── compute_indicators.py    ← P2 🔲 Build Step 2.2 (RENAME_MAP critical)
    │   ├── ingest_asm_gsm.py        ← P2 🔲 Build Step 2.3
    │   └── data_quality_monitor.py  ← P2 🔲 Build Step 2.5
    │
    ├── signals/
    │   ├── generate_signals.py      ← P0 ✅ ⚠️ PATCH Fix#4+#6, G1 REQUIRED
    │   └── independent_scanner.py   ← P1 ✅
    │
    ├── ai/
    │   ├── ai_enrich.py             ← P1 ✅ ⚠️ PATCH Fix#3, G3+G5+G6+G13+G17+G18 REQUIRED
    │   ├── ai_router.py             ← P1 ✅
    │   ├── post_trade_analysis.py   ← P1 ✅ ⚠️ PATCH Fix#2, G2 REQUIRED
    │   └── providers/
    │       ├── ml_provider.py            ← P1 ✅
    │       ├── ml_regime_classifier.py   ← P2 🔲 Build Step 2.4
    │       ├── claude_provider.py        ← P1 ✅ ⚠️ G5 patch needed
    │       ├── openai_provider.py        ← P1 ✅ ⚠️ G5 patch needed
    │       ├── gemini_provider.py        ← P1 ✅ ⚠️ G5 patch needed
    │       ├── deepseek_provider.py      ← P1 ✅
    │       └── grok_provider.py          ← P1 ✅ ⚠️ G5 patch needed
    │
    ├── alerts/
    │   └── send_alerts.py           ← P1 ✅
    │
    ...
    │
    ├── history/
    │   ├── append_history.py        ← P0 ✅ ⚠️ G14 patch needed
    │   ├── evolution_tracker.py     ← P4 ✅ PATCH Fix#5 applied (G8+G9)
    │   └── discovery_engine.py      ← P4 🔲 Build Step 4.2
    │
    └── scripts/
        └── backfill_msl_history.py  ← One-time ✅ Already run
```

**Legend:** ✅ Live · ⚠️ Live with patch needed · 🔲 Not yet built
