...
## 2026-08-24 — F-54 (new, shadow mechanism built) — Stage D3: the
event-driven core, in shadow only. Branch `feat/intraday-event-core`,
off `main` — NOTE: `feat/intraday-live-universe` (Track D Stage D2)
independently continues this same ledger from its own F-54 through
F-61, unmerged. Renumbering happens once branches actually merge; until
then each branch's own sequence is internally consistent, not globally
unique — named here rather than silently guessed around.
...
`detected_at` field. On `feat/intraday-event-core`, not merged — per the
operator's own stated plan, Gate D3 (like Gate D2 before it) is deferred
to a single holistic pass across every Track D stage, not cleared
stage-by-stage.
