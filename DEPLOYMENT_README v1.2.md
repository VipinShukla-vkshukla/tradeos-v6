...
6. [Daily Operating Procedure](#6-daily-operating-procedure)
...
- Telegram alerts
- Industry Strength scoring bonuses (stored but not scoring)
...
### Step 1.2 — Enable Industry Strength Scoring Bonuses

Industry data has been accumulating in Supabase since Phase 0. Now activate the scoring.
...
**Verify `generate_signals.py` has this block.** If it does not, add it to the scoring section:

```python
# Industry scoring bonuses — activates when industry_scoring_active = true
if config.get('industry_scoring_active') == 'true':
    if row.get('industry_top5'):
        score += 10   # stock is in top 5 industry by rank
    if row.get('industry_state') == 'STRONG':
        score += 5    # industry RSI breadth is strong
```
...
## 9. Full Repository Structure & Script Reference
